<template>
   <div class="h-[90vh]  bg-[#FBFFFB] ">
    <div class="h-full flex flex-col">
        <div class="flex items-center text-[#008B16] justify-center justify-self-start pt-16">
            <p class="w-[45px] border-b-2 border-[#008B16]"></p>
            <h2>Pricing</h2>
            <p class="w-[45px] border-b-2 border-[#008B16]"></p>
        </div>

        <p class="text-center">Our cleaning service company offers cleaning services in <br> Lagos</p> 



        
        <div class="absolute w-full h-full items-center flex justify-around">

            <div class="w-[18rem] rounded-lg shadow-xl px-5">
                <div class="flex flex-col">
                    <h2 class="text-xl py-6">Home</h2>
                    <div class="flex ">
                        <img src="~/assets/images/naira.svg" />
                        <p>50,000/month</p>
                    </div>

                    <p class="py-4">The perfect way to start our cleaning service</p>
                    <hr>
                    <div class="flex flex-col py-4 gap-2">

                    <div class="flex  items-center w-full">
                        <img src="~/assets/images/check.svg" />
                        <p>Interior cleaning and environment</p>
                    </div>

                    <div class="flex  items-center w-full">
                        <img src="~/assets/images/check.svg" />
                        <p>Interior cleaning and environment</p>
                    </div>

                    <div class="flex  items-center w-full">
                        <img src="~/assets/images/check.svg" />
                        <p>Interior cleaning and environment</p>
                    </div>

                    </div>  


                    <button class="w-full py-2 bg-[#008B16] text-white mb-6 mt-4">Get Started</button>


                </div>
            </div>

            <div class="w-[18rem] rounded-lg shadow-xl px-5 bg-[#008B16] text-white">
                <div class="flex flex-col">
                    <h2 class="text-xl py-6">Offices</h2>
                    <div class="flex ">
                        <img src="~/assets/images/nairawhite.svg" />
                        <p>100,000/month</p>
                    </div>

                    <p class="py-4">The perfect way to start our cleaning service</p>
                    <hr>
                    <div class="flex flex-col py-4 gap-2">

                    <div class="flex  items-center w-full">
                        <img src="~/assets/images/check.svg" />
                        <p>Interior cleaning and environment</p>
                    </div>

                    <div class="flex  items-center w-full">
                        <img src="~/assets/images/check.svg" />
                        <p>Interior cleaning and environment</p>
                    </div>

                    <div class="flex  items-center w-full">
                        <img src="~/assets/images/check.svg" />
                        <p>Interior cleaning and environment</p>
                    </div>

                    </div>  


                    <button class="w-full py-2 bg-[white] text-black mb-6 mt-4">Get Started</button>


                </div>
            </div>



<div class="w-[18rem] rounded-lg shadow-xl px-5">
                <div class="flex flex-col">
                    <h2 class="text-xl py-6">Home</h2>
                    <div class="flex ">
                        <img src="~/assets/images/naira.svg" />
                        <p>50,000/month</p>
                    </div>

                    <p class="py-4">The perfect way to start our cleaning service</p>
                    <hr>
                    <div class="flex flex-col py-4 gap-2">

                    <div class="flex  items-center w-full">
                        <img src="~/assets/images/check.svg" />
                        <p>Interior cleaning and environment</p>
                    </div>

                    <div class="flex  items-center w-full">
                        <img src="~/assets/images/check.svg" />
                        <p>Interior cleaning and environment</p>
                    </div>

                    <div class="flex  items-center w-full">
                        <img src="~/assets/images/check.svg" />
                        <p>Interior cleaning and environment</p>
                    </div>

                    </div>  


                    <button class="w-full py-2 bg-[#008B16] text-white mb-6 mt-4">Get Started</button>


                </div>
            </div>


        </div>


        </div>  

    </div>
</template>